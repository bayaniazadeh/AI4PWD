...
if menu == "Phase 1: Title and abstract screening":
    ...
    if uploaded_file is not None:
        ...
        # st.write(f"{len(df)} Title and Abstracts Entered")
        st.markdown(f"<p style='color:green;'>{len(df)} Title and Abstracts Entered</p>", unsafe_allow_html=True)
        ...
        if 'Title' in df.columns and 'Abstract' in df.columns:
            ...
            if st.button("Start Reviewing Titles and Abstract"):
                with st.spinner("Processing titles and abstracts..."):
                    df['result'] = df['Combined'].progress_apply(Screening_v1)

                # Display processed data
                st.success("Processing completed!")
                st.write("Processed Data Preview:", df[['Title', 'Abstract', 'result']].head())
